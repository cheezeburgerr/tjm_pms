...
export default function Check({ auth, order }) {
    ...
    const handleCheckboxChange = async (index) => {
        ...
        currentItem.status = currentItem.status === '1st Check' ? 'Printed' : '1st Check';
        setLineup(updatedLineup);

        const newProgress = calculateProgress(updatedLineup);
        setProgress(newProgress);
        try {
            router.put(route('checking.update', currentItem.id), {
                orderId: order.id,
    ...
    const calculateProgress = (lineupData) => {
        ...
        const checkedLineups = lineupData.filter(item => item.status === '1st Check').length;
    ...
    const handleFinishButtonClick = (e) => {
        const statusesToFilter = ['1st Check'];
        ...
        if (unchecked.length === 0) {
            ...
            router.put(route('employee.changestatus', order.production.id), {
                status: 'Sewing',
                role: 'Checker',
                message: 'Order Proceeded to Sewing Process'
            })
        } else {
            setShowModal(true);
        }
            ...
            <div className="columns-2 border dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 p-4 rounded-md shadow-none mb-4">
                ...
                <div className="break-inside-avoid-column">
                    <div className="my-4 relative">
                        ...
                        <p className="mt-2 text-sm text-gray-500">{progress.toFixed(2)}% Checked</p>
                    </div>
                </div>
            </div>
